			...
			.show(1000, function() {				
				 ...
				 $.api.get('user/getblacklist?_accept=application/json').success(function(response) {		
							...
							$.each(response.blacklist, function(i, profile) {						
								$(	
								  '<li id="blackuserid'+profile.id+'" type="circle"><a href="/user/'+profile.user.username+'/">'+profile.user.username+'</a> <span class="btnremove" iduser='+profile.id+'  > ('+gettext('Remove')+' )</span> </li>'					
