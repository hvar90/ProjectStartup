	  ...
	  $('#popup-map-select-classified')		
		.show(1000, function() {
			...
			function onClick(evt) {
				  ...
				  //coordinate[0]= coordinate[0] + ( 69000 / ( maxResolution / view.getResolution() ) );
				  //coordinate[1]= coordinate[1] - ( 310000 / ( maxResolution / view.getResolution() ) )	;
				  ...
				  $(element).popover({
					...
					'animation': false,
					'html': true,
					'content': '<img src="http://cdn1.socialchan.com/media/images/blank2.png" style="width:28px; height:48px;" >' 
				  });
			...
			function chooseAddr(lat1, lng1, lat2, lng2, osm_type) {
				...
				if (osm_type == "node") {				
					...
					vectorSource.addFeature(new ol.Feature(new ol.geom.Circle(position1, 25)));
					...
					//coordinate1= [position1[0] , position1[1]]	
					//coordinate2= [position2[0] , position2[1]]		
					//var points = [
						//new OpenLayers.Geometry.Point(position1[1], position1[0]),
						//new OpenLayers.Geometry.Point(position2[1], position2[0]),
						//new OpenLayers.Geometry.Point(position1[1], position2[0]),
						//new OpenLayers.Geometry.Point(position2[1], position1[0])
					//];
					...
					var points = [
						new ol.geom.Point([position1[0] , position1[1]]	),
						new ol.geom.Point([position2[0] , position2[1]]	),
						new ol.geom.Point([position1[0] , position2[1]]	),
						new ol.geom.Point([position2[0] , position1[1]]	)
					];
			...
			function addrSearch() {
				...
				$.getJSON('http://nominatim.openstreetmap.org/search?viewbox=-76.58,3.33, -76.4557, 3.53&format=json&limit=7&countrycodes=CO&Email=hvar90@gmail.com&q=' + inp.value+ ', Cali', function(data) {
					var items = [];

					$.each(data, function(key, val) {
						
						var bb = val.boundingbox;
						//items.push("<li><a class='chooseAddr' lon=" + val.lon + " lat=" + val.lat + " osm_type=" + bb[4] + " href='#' >" + val.display_name + '</a></li>');
					
						items.push("<li><a class='chooseAddr' lat1=" + bb[0] + " lng1=" + bb[2] + " lat2=" + bb[1] + " lng2=" + bb[3] + " osm_type=" + bb[4] + " href='#' >" + val.display_name + '</a></li>');
...
$(function() {
  var l = ['board', 'emoticons', 'settings', 'style', 'posts', 'hotkeys','popups', 'ajaxBoard', 'ajaxUsers', 'pubsub', 'classified'];
