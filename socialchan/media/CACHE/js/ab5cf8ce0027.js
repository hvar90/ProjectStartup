...
contacts: {
  init: function() {
	   ...
	   $(document).ready(function() {
			 if ($.cookie('show_blacklist_success') == 'true'){
				 
				 $.notification(gettext('The user has been added to the blacklist'),'success');  				 
				 $.cookie('show_blacklist_success', null);
			 }
			 
			  if ($.cookie('show_blacklist_error') == 'true'){
				 
				 $.notification(gettext('The user is already in blacklist'),'success');  				 
				 $.cookie('show_blacklist_error', null);
			 }
		});    
  }
},
			...
			.show(1000, function() {			
				 ...
				 function insermsgsents(pageNumber) {
					  ...
					  $.api.get('message/msgsentuser/?page='+current_page+'&_accept=application/json').success(function(response) {		
							    ...
							    $.each(response[0], function(i, msg) {								 				   
									$('<div class="msgbox'+ msg.id +'">\
										 <img style="border: 0.1em solid; float: right; " src="/media/images/blank2.png" alt="Profile photo" height="62" width="62"> \
										  <div class="triangle-obtuse right-t comment">\
											  <div style="float: right;">\
												 <span id="pub_date_msg">' + msg.pub_date + '</span>\
											  </div> \
											  <div style="padding-top: 25px;">' +  msg.msg_html + '</div>\
											  <div style="float: right;  padding-top: 18px;" class="">\
												 		<span to='+msg.to_id +' class="msgIcon replyMsg"><img  class="largerIcon" src="/media/images/famfamfam/icons/email2.png" title=""> '+ gettext("Send another message") +' </span>\
		  ...
		  $('#msg-received')		
			.show(1000, function() {
				...
				$('#tabs-messages').on('click','.replyMsg', function(e) {              
									 ...
									 var user = $(this).attr('from') ? $(this).attr('from') : $(this).attr('to')
				 ...
				 function insermsgsents(pageNumber) {
					  ...
					  $.api.get('message/msgreceiveduser/?page='+current_page+'&_accept=application/json').success(function(response) {								
							    ...
							    $.each(response[0], function(i, msg) {	
									...
									$(	
									   '<div class="msgbox'+ msg.id +'">\
									      <img style="border: 0.1em solid; float: left; " src="/media/images/blank2.png" alt="Profile photo" height="62" width="62"> \
										  <div  class="triangle-obtuse read-'+ msg.msg_read +' left-t comment ">\
											  <div style="float: right;">\
												 <span id="pub_date_msg">' + msg.pub_date + '</span>\
											  </div> \
											  <div style="padding-top: 25px;">' +  msg.msg_html + '</div>\
											  <div style="float: right;  padding-top: 18px;" class="">\
													...
													<span  from='+msg.fromThe_id+' class="msgIcon replyMsg"><img  class="largerIcon" src="/media/images/famfamfam/icons/email2.png" title=""> '+ gettext("Reply") +' </span>\
													<span id-msg='+msg.id+' class="msgIcon msgDel"><img  class="largerIcon" src="/media/images/famfamfam/icons/cross2.png" title="">'+ gettext("Delete message") +' </span>\
													<span from='+msg.fromThe_id+' class="msgIcon addBlackList"><img  class="largerIcon" src="/media/images/famfamfam/icons/user_delete2.png" title="">'+ gettext("Add to blacklist") +' </span>\
											   </div>\
										  </div>\
										</div>'	
									 ).appendTo('#msg-received');							
								});							
	  ...
	  $('#popup-map-select-classified')		
		.show(1000, function() {
			...
			function onClick(evt) {
				  ...
				  coordinate[0]= coordinate[0] + ( 290000 / ( maxResolution / view.getResolution() ) );
				  coordinate[1]= coordinate[1] - ( 1000000 / ( maxResolution / view.getResolution() ) )	;
...
ajax: {
  ...
  init: function() {
				...
				if ($('#checkbox_search').is(":checked"))
				  ...
				  $.api.get('section/search/'+group+'?q='+$('#q').val()+'&_accept=application/json').success(function(response) {										
							...
							$.each(response, function(i, item) {						   
								$('<li><a href="../'+item.slug+'/">/'+item.slug+'/'+item.name+'</a></li>').appendTo('#ul_search_sections');
							});
	...
	$('#received').on('click','.addBlackList', function(e) {              
				
			var idFrom= $(this).attr('from');						
		  $.api.put('user/addblacklist', JSON.parse('{"idfrom":"'+idFrom+'"}')).success(function(response) {			   
			   
			
			   $.cookie('show_blacklist_success', 'true');
			   location.href ='/user/contacts/#blacklist' ;
		 		
									 
		  }).error(function(response) {
			
			   $.cookie('show_blacklist_error', 'true');
			   location.href ='/user/contacts/#blacklist';	
		  });						
	});	
	...
	$('#received').on('click','.checkboxMsg',function() {
		...
		if (checkbox.is(':checked')) {
			var check = 'true';
			$("#msgbox"+checkbox.attr('id-msg')).css("background-color", "#F5D0A9");
			
		} else {
			var check = '';			
			$("#msgbox"+checkbox.attr('id-msg')).css("background-color", "#FCE79B");
		}
...
$(function() {
  var l = ['board', 'settings', 'style', 'posts', 'hotkeys','popups', 'ajax', 'pubsub', 'classified', 'emoticons', 'messages', 'contacts'];
