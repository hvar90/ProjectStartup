			...
			.show(1000, function() {				
				 ...
				 $.api.get('post/getpostsbyusers?username='+$('#title-username').attr('username')+'&_accept=application/json').success(function(response) {					 
					 ...
					 if (!response[0].length)
					  {
					     $(	
						    '<p>'+ gettext('You have no posts yet.') +'</p>'					
						  ).appendTo('#ul_posts');
					  }
					 else
					  {
			...
			.show(1000, function() {					
				 ...
				 $.api.get('post/getanonymouspostsbyusers?username='+$('#title-username').attr('username')+'&_accept=application/json').success(function(response) {					 
					 ...
					 if (!response[0].length)
					  {
					     $(	
						    '<p>'+ gettext('You have no anonymous posts yet.') +'</p>'					
						  ).appendTo('#ul_anonymous_post');
					  }
					 else
					  {
					    $.each(response[0], function(i, fff) {
								$(	
								  '<li >\
								       <a href="'+fff.link+'">'+fff.thread__section__name+'</a>&nbsp;&mdash;\
								       '+fff.message.slice(0,100)+'\
								   </li></br>'					
								 ).appendTo('#ul_anonymous_post');
							});	
					  }	
