...
contacts: {
  init: function() {
	   ...
	   $('#tabs-messages').on('click', '.btnremove ', function(e) {              
			...
			.show(1000, function() {				
				 ...
				 $.api.get('user/getblacklist?_accept=application/json').success(function(response) {		
							...
							$.each(response.blacklist, function(i, profile) {						
								$(	
								  '<li type="circle"><a href="/user/'+profile.user.username+'/">'+profile.user.username+'</a> <span class="btnremove" iduser='+profile.id+'  > ('+gettext('Remove')+' )</span> </li>'					
