...
ajax: {
  ...
  init: function() {
	...
	$('#menu-account').on('click', '#addcontact', function(e) {              
			 ...
			 $.api.put('user/addinvitation', JSON.parse('{"idProfile":"'+idProfile+'"}')).success(function(response) {			   
			...
			.show(1000, function() {				
				 ...
				 $.api.get('user/getblacklist?_accept=application/json').success(function(response) {		
							...
							$.each(response.blacklist, function(i, profile) {						
								$(	
								  '<li id="blackuserid'+profile.id+'" ><a href="/user/'+profile.user.username+'/">'+profile.user.username+'</a> <span class="btnremoveblacklist pointer" iduser='+profile.id+'  > ('+gettext('Remove')+' )</span> </li>'					
			...
			.show(1000, function() {				
				 ...
				 $.api.get('user/getcontacts?_accept=application/json').success(function(response) {
						...
						$.each(response.contacts, function(i, profile) {
							...
							$(	
							  '<li id="contactsid'+profile.id+'" ><img class="imgUser pointer" src="'+ photo +'" alt="Profile photo" height="62" width="62"><br><a href="/user/'+profile.user.username+'/">'+profile.user.username+'</a><img notacceptiduser='+profile.id+' class="pointer" src="/media/images/famfamfam/icons/cancel.png" title="'+gettext('Remove')+'"  height="17" width="17"> </li>'					
					 ...
					 if (!response.length)
					 ...
					 else
					  {
					    $.each(response, function(i, objuser) {
								$(	
								  '<li id="invitationcontactid'+objuser.user.id+'" ><a href="/user/'+objuser.user.username+'/">'+objuser.user.username+'</a> <img notacceptiduser='+objuser.user.id+' class="pointer btnNotAcceptContact" src="/media/images/famfamfam/icons/cancel.png" title="'+gettext('Not accept')+'"  height="17" width="17">&nbsp;&nbsp;&nbsp;<img acceptiduser='+objuser.user.id+' class="pointer btnAcceptContact" src="/media/images/famfamfam/icons/accept.png" title="'+gettext('Accept')+'"  height="17" width="17"></li>'					
								 ).appendTo('#ul_invitation_contacts');
							});	
					  }																					
