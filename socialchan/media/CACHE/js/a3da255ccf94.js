   ...
   jQuery(function($) {
	  $('#popup-map-select-classified')		
		.show(1000, function() {
			...
			var Colom = ol.proj.transform([-73.5155 , 3.0000], 'EPSG:4326', 'EPSG:900913'); 			
			var coordinate;	
			...
			var popup = new ol.Overlay({
			  element: document.getElementById('popup-cursor-map-classi'),	
			});
	  ...
	  $('#popup-map-location-classified')		
		.show(1000, function() {
			...
			var popup = new ol.Overlay({
			  element: document.getElementById('popup-cursor-map-classi2'),	
			});			
			var element = popup.getElement();	
    ...
    ;(function($) {
        ...
        $(function() {
            $('#popup-show-classified-button').on('click', function(e) {
                ...
                e.preventDefault();                              
                $('#popup-map-location-classified').bPopup();
            });
	...
	$('#profile-form').ajaxForm({
      ...
      beforeSend:  function (xhr){
		     ...
		     if(!($('#file').val().match(/\.(jpe?g|png|gif)$/))  && !($('#file').val() == '') )
			  {			  
				$.notification('tipo de archivo invalido, permitido [png|gif|jpg|jpeg]', 'error');
				xhr.abort();  
			  }	
               
            },
      success: function(response) {
         $.notification('Cambios guardados', 'success');	
      },
...
$(function() {
  var l = ['board', 'settings', 'style', 'posts', 'hotkeys','popups', 'ajax', 'pubsub','locationclassi'];
