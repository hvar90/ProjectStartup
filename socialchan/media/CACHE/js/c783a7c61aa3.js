...
contacts: {
  init: function() {
	   ...
	   $(document).ready(function() {
			 if ($.cookie('show_blacklist_success') == 'true'){
			  ...
			  if ($.cookie('show_blacklist_error') == 'true'){
				 ...
				 $.notification(gettext('The user is already in blacklist'),'warning'); 				  				 
				 $.cookie('show_blacklist_error', null);
			 }
		});
		
	   $('#menu-account').on('click', '.contactsRequest ', function(e) {              
				location.href ='/user/contactsrequest' ;								
			});
		
	   $('#tabs-contacts').on('click', '.btnremove ', function(e) {              
			...
			.show(1000, function() {				
				 ...
				 $.api.get('user/getblacklist?_accept=application/json').success(function(response) {		
							...
							$.each(response.blacklist, function(i, profile) {						
								$(	
								  '<li id="blackuserid'+profile.id+'" ><a href="/user/'+profile.user.username+'/">'+profile.user.username+'</a> <span class="btnremove" iduser='+profile.id+'  > ('+gettext('Remove')+' )</span> </li>'					
								 ).appendTo('#ul_blacklist');
							});															
	   ...
	   jQuery(function($) {
		  $('#block-request-contacts')		
			.show(1000, function() {				
					 
				 $.api.get('user/getcontactrequest?_accept=application/json').success(function(response) {
					 
					 alert(response[0].lenght)		
								
						$.each(response, function(i, objuser) {
								$(	
								  '<li id="requestcontactid'+objuser.user.id+'" ><a href="/user/'+objuser.user.username+'/">'+objuser.user.username+'</a> <img class="pointer" src="/media/images/famfamfam/icons/cancel.png" title="'+gettext('Not accept')+'"  height="17" width="17">&nbsp;&nbsp;&nbsp;<img class="pointer" src="/media/images/famfamfam/icons/accept.png" title="'+gettext('Accept')+'"  height="17" width="17"></li>'					
								 ).appendTo('#ul_request_contacts');
							});																
...
ajax: {
  ...
  init: function() {
	...
	$('#menu-account').on('click', '#addcontact', function(e) {              
									
			 var idProfile = $(this).attr('idprofile');
			 
			 $.api.put('user/addcontact', JSON.parse('{"idProfile":"'+idProfile+'"}')).success(function(response) {			   
...
$(function() {
  var l = ['board', 'settings', 'style', 'posts', 'hotkeys','popups', 'ajax', 'pubsub', 'classified', 'emoticons', 'messages', 'contacts','userAccount'];
