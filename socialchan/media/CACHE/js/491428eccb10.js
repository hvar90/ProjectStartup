	  ...
	  $('#popup-map-select-classified')		
		.show(1000, function() {
			...
			function chooseAddr(lon, lat, osm_type) {
				...
				//var position1 = new ol.LonLat(lng1,lat1).transform( fromProjection, toProjection); 
				//var position2 = new ol.LonLat(lng2,lat2).transform( fromProjection, toProjection);  
				
				var fromProjection = new OpenLayers.Projection("EPSG:4326");   // Transform from WGS 1984
				var toProjection   = new OpenLayers.Projection("EPSG:900913"); // to Spherical Mercator Projection
				var position       = new OpenLayers.LonLat(lon, lat).transform( fromProjection, toProjection);   
				
				//var loc1 = ol.proj.transform([lon,lat], "EPSG:4326", "EPSG:900913"); 		
				//var loc2 = ol.proj.transform([lng2 ,lat2], "EPSG:4326", "EPSG:900913");
				//alert()
				//var bounds = [loc1[1]/-1 ,loc2[1]/-1 ,loc1[0]/-1  ,loc2[0]/-1 ]
				...
				if (osm_type == "node") {
				    ...
				    alert(position.lon + " " +position.lat   )  
			...
			function addrSearch() {
				...
				$.getJSON('http://nominatim.openstreetmap.org/search?viewbox=-76.58,3.33, -76.4557, 3.53&format=json&limit=5&countrycodes=CO&Email=hvar90@gmail.com&q=' + inp.value+ ', Cali', function(data) {
					var items = [];
					...
					$('#resultsPlaces').empty();
					if (items.length != 0) {
						$('<p>', { html: "Search results:" }).appendTo('#resultsPlaces');
						$('<ul/>', {
							'class': 'my-new-list',
							html: items.join('')
						}).appendTo('#resultsPlaces');
					} else {
						$('<p>', { html: "No results found" }).appendTo('#resultsPlaces');
					}
				});
			}
