	    ...
	    jQuery(function($) {
		  $('#msg-user')		
			.show(1000, function() {
				 ...
				 function insermsgsent(e) {
					 
					  var current_page =  e ? e.target.getAttribute('page') : 1	
					  alert(current_page)
					  $.api.get('message/searchbyuser/?page='+current_page+'&_accept=application/json').success(function(response) {										
											
							    $.each(response[0], function(i, msg) {						   
									$('<li class="ccc">'+msg.msg+'</li>').appendTo('#msg-sent');
								});
								
								$.each(response[1], function(i, pag) {		
									if(pag != response[2])
									{
									 // $('<li><a class = ccc  page='+pag+'  >'+pag+'</a> </li>').appendTo('#pag-msg-sent');	
									  $('#pag-msg-sent').append('<li><a class="ccc">'+pag+'</a></li>');						
									}
									else	
									{										
									  $('<li  class="ccc"><span  class="ccc">'+pag+'</span></li>').appendTo('#pag-msg-sent');	
									}		   
								});
