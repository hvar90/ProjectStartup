  ...
  init: function() {
    ...
    $('.newpost form').fileupload({
	    ...
	    success: function(response) {
		 ...
		 error: defaultErrorCallback,		
		 always: function(){ $(".newpost form .table tr").remove(); $(".newpost .start").toggle();},  // se realiza para que no muestre la interfaz de eliminacion de imagenes
		 dataType: 'json',          
         url: $.api.url + 'post/?html=1&_accept=application/json',        
        })
    });
    
    $('.newpost form').serializeArray()
    $( '#files').on('change', function(){ $(".newpost form .table tr").remove();});
    $( '#files').on('change', function(){ $(".newpost form .table tr").remove();  $(".newpost .start").toggle();});
