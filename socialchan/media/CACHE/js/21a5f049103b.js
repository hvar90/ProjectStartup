...
ajaxUsers: {
  init: function() {
	...
	$('#menu-account').on('click', '#sendInvitation', function(e) {              
			 ...
			 $.api.put('user/'+idAuthenticUser+'/?sendinvitation', JSON.parse('{"idProfile":"'+idProfile+'"}')).success(function(response) {			   
    ...
    $('.footer-link-img a').click(function() { 
      ...
      $.api.put('user/profile/?removeFile').error(function(xhr) {
	...
	$('#received, #menu-account').on('click','.addBlackList', function(e) {              
		  ...
		  var idAuthenticUser= $(this).attr('authenticUser');						
		  $.api.put('user/'+idAuthenticUser+'?addBlackList', JSON.parse('{"idfrom":"'+idFrom+'"}')).success(function(response) {		
    ...
    $('#tabs-contacts').on('click', '.btnremoveblacklist ', function(e) {              
				 ...
				 $.api.put('user/'+idAuthenticUser+'/?removeBlacklist',JSON.parse('{"iduser":"'+iduser+'"}')).success(function(response) {		
	...
	jQuery(function($) {
		  $('#blacklist')			  
			.show(1000, function() {				
				 var idAuthenticUser= $('#tabs-contacts').attr('authenticUser');			 
				 $.api.get('user/'+idAuthenticUser+'?_accept=application/json').success(function(response) {		
							var tmp = '';	
					    ...
					    $.each(response, function(i, objuser) {
								tmp +='<li id="invitationcontactid'+objuser.user.id+'" >\
										  <h2>\
											 ...
											 <img notacceptiduser='+objuser.user.id+' class="pointer btnNotAcceptContact" src="http://cdn1.socialchan.com/media/images/famfamfam/icons/cancel.png" title="'+gettext('Not accept')+'"  height="17" width="17">&nbsp;&nbsp;&nbsp;<img acceptiduser='+objuser.user.id+' class="pointer btnAcceptContact" src="http://cdn1.socialchan.com/media/images/famfamfam/icons/accept.png" title="'+gettext('Accept')+'"  height="17" width="17">\
										  </h2>\
								      </li>';					 
							});	
	...
	$('#block-invitation-contacts').on('click','.btnNotAcceptContact',function() {	
		...
		$("#invitationcontactid" + iduser).fadeOut(300, function() { $(this).remove(); });  
		$.api.put('user/denyinvitation', JSON.parse('{"iduser":"'+iduser+'"}')).success(function(response) {				
