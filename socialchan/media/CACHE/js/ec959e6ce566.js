	  ...
	  $('#popup-map-show-classified ')		
		.show(1000, function() {
			...
			$('.popup-show-classified-button').on('click', function(e) {
			    ...
			    var vectorSource = new ol.source.Vector();
				vectorSource.addFeature(new ol.Feature(new ol.geom.Circle( [parseFloat(lon)  , parseFloat(lat)], 25)));
                ...
                view = map.getView();              
                view.setCenter([parseFloat(lon) , parseFloat(lat)]); // se divide entre 1 para solucionar bug de OL3
                view.setZoom(zoom);
                map.addLayer(vectorLayer);   
                
                $(element).popover('destroy');
				popup.setPosition([parseFloat(lon) , parseFloat(lat)]);		
				$(element).popover({
					'placement': 'bottom',
					'animation': false,
					'html': true,
