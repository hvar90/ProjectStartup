...
messages: {
  init: function() {
	   ...
	   $(document).ready(function() {
		...
		$('.pagination_msgsent').on('click', function(e) {	 
				 ...
				 function insermsgsent(e) {
					  ...
					  $.api.get('message/searchbyuser/?page='+current_page+'&_accept=application/json').success(function(response) {										
							    ...
							    $.each(response[0], function(i, msg) {						   
									$('<li>'+msg.msg+'</li>').appendTo('#msg-sent');
								});
								
								$.each(response[1], function(i, pag) {		
									if(pag != response[2])
									{
									  $('<li class="pagination_msgsent" ><a class="pagination_msgsent"  page='+pag+'  >'+pag+'</a> </li>').appendTo('#pag-msg-sent');							
									}
									else	
									{
										
									  $('<li><span>'+pag+'</span></li>').appendTo('#pag-msg-sent');	
									}		   
								});
