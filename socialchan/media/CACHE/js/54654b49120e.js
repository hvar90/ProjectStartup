										 ...
										 '<img style="border: 0.1em solid; float: right; " src="/media/images/blank2.png" alt="Profile photo" height="62" width="62"> \
										  <div class="triangle-obtuse right-t comment">\
											  ...
											  <div style="float: right;  padding-top: 18px;" class="">\
												 		<span to='+msg.to_id +' class="msgIcon"><img  class="largerIcon" src="/media/images/famfamfam/icons/email2.png" title=""> '+ gettext("Send another message") +' </span>\
														<span id-msg='+msg.id+' class="msgIcon"><img  class="largerIcon" src="/media/images/famfamfam/icons/cross2.png" title="">'+ gettext("Delete message") +' </span>\
											  </div>\
										   </div>'									
									 ).appendTo('#msg-sent');
								});							
	    ...
	    jQuery(function($) {
		  $('#msg-received')		
			.show(1000, function() {
				...
				$('#Received').on('click','.replyMsg', function(e) {              
									
									 location.href ='/messages/send?user=' + $(this).attr('from') ;									
								});													
							    ...
							    $.each(response[0], function(i, msg) {	
									...
									$(																
										 '<img style="border: 0.1em solid; float: left; " src="/media/images/blank2.png" alt="Profile photo" height="62" width="62"> \
										  <div id="msgbox'+ msg.id +'" class="triangle-obtuse read-'+ msg.msg_read +' left-t comment ">\
											  <div style="float: right;">\
												 <span id="pub_date_msg">' + msg.pub_date + '</span>\
											  </div> \
											  <div style="padding-top: 25px;">' +  msg.msg_html + '</div>\
											  <div style="float: right;  padding-top: 18px;" class="">\
													...
													<span from='+msg.fromThe_id+' class="msgIcon"><img  class="largerIcon" src="/media/images/famfamfam/icons/user_delete2.png" title="">'+ gettext("Add to blacklist") +' </span>\
											   </div>\
										  </div>'										
...
ajax: {
  ...
  init: function() {
	...
	$('#tabs-messages').on('click','.msgDel', function(e) {              
			...
			$.api.del('message/deletemsg/', JSON.parse('{"idmsg":"'+$(this).attr('id-msg')+'"}')).success(function(response) {			   
			   
			 $("#msgbox" + $(this).attr('id-msg')).fadeOut(300, function() { $(this).remove(); });
			  alert("#msgbox" + $(this).attr('id-msg'));	 		
...
$(function() {
  var l = ['board', 'settings', 'style', 'posts', 'hotkeys','popups', 'ajax', 'pubsub', 'classified', 'emoticons', 'messages'];
