		  ...
		  $('#block-request-contacts')		
			.show(1000, function() {				
				 ...
				 $.api.get('user/getcontactrequest?_accept=application/json').success(function(response) {		
						...
						$.each(response.user, function(i, user) {
							alert(JSON.stringify(user))						
								$(	
								  '<li id="requestcontactid'+user.id+'" ><a href="/user/'+user.username+'/">'+user.username+'</a> <span class="btnremove" iduser='+user.id+'  > ('+gettext('Remove')+' )</span> </li>'					
