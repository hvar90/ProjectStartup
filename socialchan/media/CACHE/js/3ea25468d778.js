			...
			.show(1000, function() {			
				 ...
				 function insermsgsents(pageNumber) {
					  ...
					  $.api.get('message/msgsentuser/?page='+current_page+'&_accept=application/json').success(function(response) {		
							    ...
							    $.each(response.objects, function(i, msg) {
									if ( response.objects.message_receive.length == 0  ) 
									    {return true;} 									
									var photo = msg.message_receive[0].to.profile_picture ? /files/+msg.message_receive[0].to.profile_picture : "/media/images/users/user2.png"							 				   
									tmp_msgs += '<div class="msgbox'+ msg.id +'">\
										 <div  username="'+ msg.message_receive[0].to.user.username +'" class="imgUserBoxright" >\
										   <img class="imgUser" src="'+ photo +'" alt="Profile photo" height="62" width="62"> \
										   <div class="textImgUser">'+ msg.message_receive[0].to.user.username +'</div>\
										 </div>\
										  <div class="triangle-obtuse right-t comment">\
											  ...
											  <div class="footerMsg" >\
												 		<span to='+msg.message_receive[0].to.id +' class="msgIcon replyMsg"><img  class="largerIcon" src="/media/images/famfamfam/icons/email2.png" title=""> '+ gettext("Send another message") +' </span>\
				 ...
				 function insermsgrecv(pageNumber) {
					  ...
					  $.api.get('message/msgreceiveduser/?page='+current_page+'&_accept=application/json').success(function(response) {								
								var tmp_msgs = '';
							    $.each(response.objects, function(i, msg) {	
									if ( response.objects.message_send.length == 0 ) {return true;} 									     
									var checked =  msg.msg_read  ? 'checked' : ' ';		
									var photo = msg.message_send[0].fromThe.profile_picture ? /files/+msg.message_send[0].fromThe.profile_picture : "/media/images/users/user2.png";										
								    tmp_msgs += '<div class="msgbox'+ msg.id +'">\
									      <div  username="'+ msg.message_send[0].fromThe.user.username +'" class="imgUserBoxleft">\
										     <img class="imgUser" src="'+ photo +'" alt="Profile photo" height="62" width="62"> \
										     <div class="textImgUser">'+ msg.message_send[0].fromThe.user.username +'</div>\
										  </div>\
										  <div  id="msgcontent'+ msg.id +'"  class="triangle-obtuse read-'+ msg.msg_read +' left-t comment ">\
											  ...
											  <div class="footerMsg">\
													...
													<span  from='+msg.message_send[0].fromThe.id+' class="msgIcon replyMsg"><img  class="largerIcon" src="/media/images/famfamfam/icons/email2.png" title=""> '+ gettext("Reply") +' </span>\
													<span id-msg='+msg.id+' class="msgIcon msgDel"><img  class="largerIcon" src="/media/images/famfamfam/icons/cross2.png" title="">'+ gettext("Delete message") +' </span>\
													<span from='+msg.message_send[0].fromThe.id+' class="msgIcon addBlackList"><img  class="largerIcon" src="/media/images/famfamfam/icons/user_delete2.png" title="">'+ gettext("Add to blacklist") +' </span>\
											   </div>\
										  </div>\
										</div>'	;
