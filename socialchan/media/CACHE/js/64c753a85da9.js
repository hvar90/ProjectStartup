   ...
   jQuery(function($) {
	  $('#popup-map-select-classified')		
		.show(1000, function() {
			...
			var feature;
			function chooseAddr(lon, lat, osm_type) {
				...
				if (osm_type == "node") {
					...
					//map.on('postcompose', function(evt) {
						  ...
						  //evt.vectorContext.drawCircleGeometry(
							  //new ol.geom.Circle([position.lon,position.lat], 400000));
						//});
		
					
					//feature = L.polyline( [loc1, loc4, loc2, loc3, loc1], {color: 'red'});				
					//map.fitBounds(bounds);
				    var view2 = map.getView() 
				    view2.setZoom(18);				 
					view2.setCenter([position.lon,position.lat]);
					var vectorSource = new ol.source.Vector();
					vectorSource.addFeature(new ol.Feature(new ol.geom.Circle([position.lon, position.lat], 1e6)));
					
					vectorLayer.set('source',vectorSource)
			...
			$( "#resultsPlaces" ).on('click',".chooseAddr", function() {
				...
				chooseAddr($( this ).attr( "lon" ),$( this ).attr( "lat" ),$( this ).attr( "osm_type" ))
			});	
