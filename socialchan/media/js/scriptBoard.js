...
$.extend({
  settings: function(name, value) {
    ...
    if (typeof name === 'undefined') {
      ...
      if (value === 'false' || value === false || value === '' || value === 'on') {
        value = '';
        type = 'del';
        url += data.key;
      }
...
board: {
  ...
  init: function() { 
    ...
    // Posts deletion
    $('.threads').on('click','.delPostButton',  function(event) {
    
      var target= $("[data-id='"+$( this ).attr( "target-post-id" )+"']" )  
      var password;
      var onlyFiles = !!$('#only_files').attr('checked');
      var banIp = !!$('#ban_ip').attr('checked');
      var deleteAll = !!$('#delete_all').attr('checked');    
      var targetFiles = $('#gallery'+target.data('id'));
      var url = !onlyFiles ?
          'post/' + target.data('id') :
          'file/' + target.data('id') ;//se cambio target.data('id') por que ahora son varios archivos por post
    
      password = $('.password-d #password').val();      
      url += '?password=' + password;
      url += '&' + $('.removePosts').serialize();
      if (onlyFiles) {
		   targetFiles.addClass('deleted');
    ...
    $('.threads').delegate('.number > a', 'click', function(e) {
      ...
      if (curPage.type === 'page' || curPage.type === 'thread') {
        if (!$.settings('disableQuickReply')) {
          ...
          $('.newpost').insertAfter(n.selector);
...
settings: {
  init: function() {
    ...
    //se comento por que por el momento no se permite material pornografico en el sitio
...
share: {
  init: function() {	  
	   ...
	   function postToFb(body) {
	  	...
	  	$('.FbButton, .FbButtonShare').on('click', function(e) {              
		...
		.show(1000, function() {
			...
			function onClick(evt) {
				  ...
				  $(element).popover({
					...
					'animation': false,
					'html': true,
					'content': '<img src="'+STATIC_URL+'images/blank2.png" style="width:28px; height:48px;" >' 
				  });
			...
			$( "#accept-location" ).on('click', function() {
				  ...
				  $( "#location" ).val( '{"location": {"lon": "'+coordinate[0]+'", "lat": "'+coordinate[1]+'", "zoom": "'+view.getZoom()+'"}}' );
			...
			function chooseAddr(lat1, lng1, lat2, lng2, osm_type) {
				...
				var position1       = new OpenLayers.LonLat(lng1, lat1).transform(fromProjection, toProjection); 
				var position2       = new OpenLayers.LonLat(lng2, lat2).transform(fromProjection, toProjection); 
				var bounds = [position1[0] ,position2[0] ,position1[1] ,position2[1]]; 
				...
				if (osm_type == "node") {
				    ...
				    var view = map.getView();				  
				    view.setZoom(18);				 
					view.setCenter([position1.lon , position1.lat]);			
					var vectorSource = new ol.source.Vector();
					vectorSource.addFeature(new ol.Feature(new ol.geom.Circle( [position1.lon , position1.lat], 25)));					
					...
					var view = map.getView(); 
				    view.setZoom(18);				 
					view.fitExtent([position1.lon,position1.lat,position2.lon,position2.lat],map.getSize());					
					var points = [
						[position1.lon , position1.lat],					
						[position1.lon , position2.lat],
						[position2.lon , position2.lat],
						[position2.lon , position1.lat]
						];
			...
			function addrSearch() {
				...
				$.getJSON('http://nominatim.openstreetmap.org/search?format=json&limit=7&&Email=hvar90@gmail.com&q=' + inp.value, function(data) {
					var items = [];

					$.each(data, function(key, val) {
						
						var bb = val.boundingbox;					
						items.push("<li><a class='chooseAddr' lat1=" + bb[0] + " lng1=" + bb[2] + " lat2=" + bb[1] + " lng2=" + bb[3] + " osm_type=" + val.osm_type + " href='#' >" + val.display_name + '</a></li>');
						
					});

					$('#resultsPlaces').empty();
					if (items.length != 0) {
						$('<p>', { html:  gettext("  Search results:")  }).appendTo('#resultsPlaces');
						$('<ul/>', {
							'class': 'list_places',
							html: items.join('')
						}).appendTo('#resultsPlaces');
					} else {
						$('<p>', { html:  gettext("  No results found") }).appendTo('#resultsPlaces');
					}
				});
			}
			
			$( "#searchPlaces" ).submit( function() {
				addrSearch()
				return false; 
			});	
			
			//$("#addr_search").keypress(function(e) {
				//if(e.which == 13) {
					//alert('You pressed enter!');
				//}
			//});
			
			$( "#resultsPlaces" ).on('click',".chooseAddr", function() {
				
				chooseAddr($( this ).attr( "lat1" ),$( this ).attr( "lng1" ),$( this ).attr( "lat2" ),$( this ).attr( "lng2" ),$( this ).attr( "osm_type" ))
			});	
	  ...
	  $('#popup-map-show-classified ')		
		.show(1000, function() {
			...
			$('.popup-show-classified-button').on('click', function(e) {
                ...
                var zoom = Number(e.target.getAttribute('zoom'));  
                if (vectorLayer) {
					map.removeLayer(vectorLayer);
				}
			    var vectorSource = new ol.source.Vector();
				vectorSource.addFeature(new ol.Feature(new ol.geom.Circle( [lon,lat], 25)));
...
ajaxBoard: {
  ...
  init: function() {
    ...
    $('#btn_follow_group').on('click', function(e) {  
		 
		 var idSection = $(this).attr('idSection');		 
		 var idProfile = $(this).attr('idProfile');		 
		 
		 $.api.put('user/profile/'+idProfile+'/?subscribe',JSON.parse('{"idSection":"'+idSection+'"}')).success(function(response) {
	...
	$('#btn_unfollow_group').on('click', function(e) {  
		 
		 var idSection = $(this).attr('idSection');		 
		 var idProfile = $(this).attr('idProfile');		 
		 
		 $.api.put('user/profile/'+idProfile+'/?unsubscribe',JSON.parse('{"idSection":"'+idSection+'"}')).success(function(response) {
    ...
    $('.newpost form').fileupload({
	    ...
	    beforeSend: function(xhr, settings) {		
			$("#overlay_loading img").show();
			 app.ajaxBoard.activatedPusub = false;
		},
		  ...
		  $('.threads ')		
			.show(1000, function() {
				...
				//if (window.CURRENT_USER) {
					  ...
					  //$.api.put('user/profile/'+window.CURRENT_USER+'/?markReadNewPosts',JSON.parse('{"idSection":"'+window.ID_SECTION+'"}')).success(function(response) {
  ...
  success: function(data) {   
    if (curPage.type !== 'thread' && !app.ajaxBoard.quickReplied) { // redirect
      window.location.href = './' + data.pid +'#post' + data.pid  ;
      return true;
    }
    
     //console.log( data);
    if (curPage.type !== 'thread' ) {
		 var placeToappend = $("section[data-id='"+data.thread+"']");
	  } 
	  else{
		 var placeToappend = $("section[data-id='"+curPage.thread+"']");
	  }  
...
pubsub: {
  ...
  poll: function() {
    ...
    console.log( curPage.section);
    $.api.post('stream/'+ curPage.section).error(function() {
      if (app.pubsub.sleepTime < app.pubsub.maxSleepTime) {
		  
		  if (curPage.section == 'chat') {				
			 app.pubsub.sleepTime = 0;
		  } else {
      ...
      //console.log(response.idThread);
      if (curPage.type !== 'thread' ) {
		 var placeToappend = $("section[data-id='"+response.idThread+"']");
		 //console.log(response.idThread)
	  } 
	  else{
		 var placeToappend = $("section[data-id='"+curPage.thread+"']");
		 //console.log(curPage.thread)
	  } 
	  if (app.ajaxBoard.activatedPusub ) {
      for (var i = 0, l = posts.length; i < l; i++) {
        var $p = $(posts[i]);
        //console.log($p, 'new msgs');
      ...
      //console.log(curPage.first, 'new msgs');
      app.pubsub.showNewPostNotification(text, curPage.section, curPage.first);
      window.location.hash = $p[0].id;
      }
      app.ajaxBoard.activatedPusub = true;
...
$(function() {
  var l = ['board', 'emoticons', 'settings', 'style', 'posts', 'hotkeys','popups', 'ajaxBoard', 'ajaxUsers', 'classified', 'pubsub','share'];
