...
share: {
  init: function() {	  
	  ...
	  (function(d, s, id){
	   ...
	   function postToFb(body) {
		   
			FB.api('/me/feed', 'post', { message: body,link:'www.socialchan.net', picture:'https://s3-us-west-2.amazonaws.com/filesocialchan1/media/images/logo2.png',height:55,width:55 }, function(response) {
				  if (!response || response.error) {
					//alert('Error occured');
				  } else {				
					$.notification(gettext('was successfully shared'),'success'); 
				  }
				});	
		}
	  	...
	  	$('.FbButton').on('click', function(e) {              
			   ...
			   var body = e.target.getAttribute('data-post');	
...
ajaxUsers: {
  init: function() {
	...
	$('#menu-account').on('click', '#sendInvitation', function(e) {              
			 ...
			 var idProfileToInvite = $(this).attr('idprofile');
			 var idAuthenticUser= $(this).attr('authenticUser');	
			 $.api.put('user/profile/'+idAuthenticUser+'/?sendinvitation', JSON.parse('{"idProfile":"'+idProfileToInvite+'"}')).success(function(response) {			   
	...
	$('#register').submit(function() { 	
		  ...
		  $.api.post('user/', postData).success(function(response) {			    
			    location.href = "/user/"+postData[0].value	
			    //location.reload() 										 
		  }).error(function(response) {
	...
	$('#menu-account').on('click','.addBlackList', function(e) {              
				
		  var idToAddBlacklist= $(this).attr('from');			
		  var idAuthenticUser= $(this).attr('authenticUser');
							
		  $.api.put('user/profile/'+idAuthenticUser+'?addBlackList', JSON.parse('{"idfrom":"'+idToAddBlacklist+'"}')).success(function(response) {		
	...
	$('#tabs-messages').on('click','.addBlackList', function(e) {              
				
		  var idToAddBlacklist= $(this).attr('from');			
		  var idAuthenticUser= $('#tabs-messages').attr('authenticUser');
    ...
    $('#tabs-contacts').on('click', '.btnremoveblacklist ', function(e) {              
				 ...
				 var idToRemoveBlacklist = $(this).attr('iduser');
				 var idAuthenticUser= $('#tabs-contacts').attr('authenticUser');	
				 $.api.put('user/profile/'+idAuthenticUser+'/?removeBlacklist',JSON.parse('{"iduser":"'+idToRemoveBlacklist+'"}')).success(function(response) {		
								
					  $.notification(gettext('The user has been eliminated from the blacklist'),'success');
					  $("#blackuserid" + idToRemoveBlacklist).fadeOut(300, function() { $(this).remove(); });													
	...
	jQuery(function($) {
	 ...
	 $('#tabs-contacts').on('click', '.removContact', function(e) {              
									
				 var idProfileToRemove = $(this).attr('removecontactid');					
				 var idAuthenticUser =  $('#tabs-contacts').attr('authenticUser');
				
				 
				 $.api.put('user/profile/'+idAuthenticUser+'/?removecontact',JSON.parse('{"iduser":"'+idProfileToRemove+'"}')).success(function(response) {								
	 ...
	 $('#menu-account').on('click', '.removContact', function(e) {              
									
				 var idProfileToRemove = $(this).attr('idprofile');						
				 var idAuthenticUser = $(this).attr('authenticUser');
				 ...
				 $.api.get('user/profile/'+idAuthenticUser+'/?&_accept=application/json').success(function(response) {
					  ...
					  { 
						...
						$.each(response.contacts, function(i, profile) {
							...
							var photo = profile.profile_picture ? MEDIA_URL+profile.profile_picture : STATIC_URL+'images/users/user2.png';												
							tmp +='<li id="contactsid'+profile.id+'" >\
									   ...
									   <a href="/user/'+profile.user.username+'/">'+profile.user.username+'</a>\
									   <img removecontactid='+profile.id+' class="pointer removContact" src="'+STATIC_URL+'images/famfamfam/icons/cancel.png" title="'+gettext('Remove contact')+'"  height="17" width="17">\
							       </li>';					
	...
	jQuery(function($) {
		  $('#results_search_members')		
			.show(1000, function() {					 
				 	 
				 $.api.get('user/profile/?getmembers&_accept=application/json').success(function(response) {
					...
					$.each(response, function(i, profile) {
						...
						var photo = profile.profile_picture ? MEDIA_URL+profile.profile_picture : STATIC_URL+'images/users/user2.png';												
						tmp +='<li>\
								   <a href="/user/'+profile.user__username+'/"><img  username="'+ profile.user__username +'" class="imgUserContact pointer" src="'+ photo +'" alt="Profile photo" height="62" width="62"></a><br>\
								   <a href="/user/'+profile.user__username+'/">'+profile.user__username+'</a>\
							   </li>';					
						
					});	
					$(tmp).appendTo('#ul_search_members');
		...
		$('#search_members').submit(function() {		
		$('#ul_search_members').empty();
		if(($('#q').val().length > 0))
			{  				
				  $.api.get('users/search/?q='+$('#q').val()+'&_accept=application/json').success(function(response) {	
					  
					if (!response.length)
					  {
					     $(	
						    '<p>'+ gettext('no results found.') +'</p>'					
						  ).appendTo('#ul_search_members');
					  }
					 else
					  {
						var tmp = '';  
	...
	jQuery(function($) {
		  $('#posts')		
			.show(1000, function() {				
				 ...
				 $.api.get('user/profile/'+$('#title-username').attr('username')+'/posts/?getposts&_accept=application/json').success(function(response) {					 
	 ...
	 jQuery(function($) {
		  $('#anonymous_posts')		
			.show(1000, function() {					
				 ...
				 $.api.get('user/profile/'+$('#title-username').attr('username')+'/posts/?getpostanonymous&_accept=application/json').success(function(response) {					 
					 ...
					 else
					  {
					      ...
					      $.each(response, function(i, section) {						
								tmp +='<li class="removsection"  id="sectionid'+section.id+'" >\
										  ...
										  <img removesectionid='+section.id+' class="pointer imgremovsection" src="'+STATIC_URL+'images/famfamfam/icons/cancel.png" title="'+gettext('Remove group')+'"  height="17" width="17">\
										  <a href="/editsection/'+section.id+'"><img editsectionid='+section.id+' class="pointer imgeditsection" src="'+STATIC_URL+'images/famfamfam/icons/layout_edit.png" title="'+gettext('Edit group')+'"  height="17" width="17"></a>\
								       </li>';								
							});
							
						$(tmp).appendTo('#ul_groups');		
							
						 $(".imgremovsection, .imgeditsection").hide();
						 $(".removsection")
							  .mouseover(function() {										
								$( "img", this ).show();
	 ...
	 jQuery(function($) {
		  $('#block-invitation-contacts')		
			.show(1000, function() {				
				 ...
				 $.api.get('user/profile/?pk='+idAuthenticUser+'&getcontactinvitation&_accept=application/json').success(function(response) {
					 if (!response.length)
					  {
					     $(	
					 ...
					 else
					  {
						var tmp = '';    
					    $.each(response, function(i, profile) {
								tmp +='<li id="invitationcontactid'+profile.id+'" >\
										  <h2>\
											 <a href="/user/'+profile.user__username+'/">'+profile.user__username+'</a>\
											 <img notacceptiduser='+profile.id+' class="pointer btnNotAcceptContact" src="'+STATIC_URL+'images/famfamfam/icons/cancel.png" title="'+gettext('Not accept')+'"  height="17" width="17">&nbsp;&nbsp;&nbsp;<img acceptiduser='+profile.id+' class="pointer btnAcceptContact" src="'+STATIC_URL+'images/famfamfam/icons/accept.png" title="'+gettext('Accept')+'"  height="17" width="17">\
										  </h2>\
								      </li>';					 
							});	
	...
	$('#block-invitation-contacts').on('click','.btnAcceptContact',function() {	
		...
		var idToAccept=  $(this).attr('acceptiduser');	
		var idAuthenticUser= $('#block-invitation-contacts').attr('authenticUser');				 
		$("#invitationcontactid" + idToAccept ).fadeOut(300, function() { $(this).remove(); });  
		
		$.api.put('user/profile/'+idAuthenticUser+'/?acceptContact', JSON.parse('{"iduser":"'+idToAccept+'"}')).success(function(response) {	
	...
	$('#block-invitation-contacts').on('click','.btnNotAcceptContact',function() {	
		...
		var idToNotAccept=  $(this).attr('notacceptiduser');
		var idAuthenticUser= $('#block-invitation-contacts').attr('authenticUser');	
				 
		$("#invitationcontactid" + idToNotAccept).fadeOut(300, function() { $(this).remove(); });  
		$.api.put('user/profile/'+idAuthenticUser+'/?denyinvitation', JSON.parse('{"iduser":"'+idToNotAccept+'"}')).success(function(response) {				
	...
	$('#change_pass_form').on('submit', function(e) {              
         e.preventDefault()
         var idAuthenticUser= $(this).attr('authenticUser');	
		 $.api.post('user/'+idAuthenticUser+'/',$(this).serializeArray()).success(function(response) {								
	...
	$('#del_account_form').on('submit', function(e) {              
         e.preventDefault()
         var idAuthenticUser= $(this).attr('authenticUser');
		 $.api.del('user/'+idAuthenticUser+'/',$(this).serializeArray()).success(function(response) {								
...
ajaxMessages: {
  init: function() {	
    $('#send_message').submit(function() { 			
		  ...
		  $.api.post('message/', postData).success(function(response) {			   
		  ...
		  $('#tabs-messages')		
			.show(1000, function() {			
				 function insermsgsents(pageNumber) {					 
					  ...
					  var idAuthenticUser= $('#tabs-messages').attr('authenticUser');	
					  $("#msg-sent").html("");						  		
					  $.api.get('message/profile/'+idAuthenticUser+'/?msgsentuser&page='+current_page+'&_accept=application/json').success(function(response) {		
						  		var tmp_msgs = '';	
							    $.each(response.msgs, function(i, msg) {
													
									var photo = msg.to__profile_picture ? MEDIA_URL+msg.to__profile_picture : STATIC_URL+"images/users/user2.png";						 				   
									tmp_msgs += '<div class="msgbox'+ msg.id +'">\
										 <div  username="'+ msg.to__user__username +'" class="imgUserBoxright" >\
										   <img class="imgUser" src="'+ photo +'" alt="Profile photo" height="82" width="102"> \
										   <span>'+ msg.to__user__username +'</span>\
										 </div>\
										  <div class="triangle-obtuse right-t comment">\
											  ...
											  <div class="footerMsg" >\
												 		<span to='+msg.to +' class="msgIcon replyMsg"><img  class="largerIcon" src="'+STATIC_URL+'images/famfamfam/icons/email2.png" title=""> '+ gettext("Send another message") +' </span>\
														<span id-msg='+msg.id+' class="msgIcon msgDel"><img  class="largerIcon" src="'+STATIC_URL+'images/famfamfam/icons/cross2.png" title="">'+ gettext("Delete message") +' </span>\
											  </div>\
										   </div>\
										 </div>';
								});							
				 ...
				 function insermsgrecv(pageNumber) {
					  ...
					  var idAuthenticUser= $('#tabs-messages').attr('authenticUser');		
					  $("#msg-received").html("");				
					  $.api.get('message/profile/'+idAuthenticUser+'/?msgreceiveduser&page='+current_page+'&_accept=application/json').success(function(response) {								
								var tmp_msgs = '';
							    $.each(response.msgs, function(i, msg) {																		     
									var checked =  msg.msg_read  ? 'checked' : ' ';		
									var photo = msg.fromThe__profile_picture ? MEDIA_URL+msg.fromThe__profile_picture : STATIC_URL+'images/users/user2.png';										
								    tmp_msgs += '<div class="msgbox'+ msg.id +'">\
												  <div  username="'+ msg.fromThe__user__username +'" class="imgUserBoxleft">\
													 <img class="imgUser" src="'+ photo +'" alt="Profile photo" height="82" width="102"> \
													 <span>'+ msg.fromThe__user__username +'</span>\
												  </div>\
												  <div  id="msgcontent'+ msg.id +'"  class="triangle-obtuse read-'+ msg.msg_read +' left-t comment ">\
													  ...
													  <div class="footerMsg">\
															...
															<span  from='+msg.fromThe+' class="msgIcon replyMsg"><img  class="largerIcon" src="'+STATIC_URL+'images/famfamfam/icons/email2.png" title=""> '+ gettext("Reply") +' </span>\
															<span id-msg='+msg.id+' class="msgIcon msgDel"><img  class="largerIcon" src="'+STATIC_URL+'images/famfamfam/icons/cross2.png" title="">'+ gettext("Delete message") +' </span>\
															<span from='+msg.fromThe+' class="msgIcon addBlackList"><img  class="largerIcon" src="'+STATIC_URL+'images/famfamfam/icons/user_delete2.png" title="">'+ gettext("Add to blacklist") +' </span>\
													   </div>\
												  </div>\
												</div>';
	...
	$('#received').on('click','.msgDel', function(e) {              
		  ...
		  var idAuthenticUser= $('#tabs-messages').attr('authenticUser');					
		  $.api.del('message/'+idmsg+'/profile/'+idAuthenticUser+'/?deletemsgreceive').success(function(response) {			   
	...
	$('#sent').on('click','.msgDel', function(e) {              
			...
			var idmsg= $(this).attr('id-msg');	
			var idAuthenticUser= $('#tabs-messages').attr('authenticUser');					
			$.api.del('message/'+idmsg+'/profile/'+idAuthenticUser+'/?deletemsgsend').success(function(response) {			   
	...
	$('#confirm_clear_msg_send').on('click', function(e) {   			
		  ...
		  $.api.del('message/profile/'+idAuthenticUser+'/?clearsendall').success(function(response) {			   
	...
	$('#confirm_clear_msg_receive').on('click',function(e) {  		
		  var idAuthenticUser= $('#tabs-messages').attr('authenticUser');							
		  $.api.del('message/profile/'+idAuthenticUser+'/?clearreceiveall').success(function(response) {			   
   ...
   $('#received').on('click','.checkboxMsg',function() {
		...
		$.api.put('message/'+checkbox.attr('id-msg')+'/profile/'+idAuthenticUser+'/', JSON.parse('{"value":"'+check+'" }')).success(function(response) {			   
...
ajaxBoard: {
  init: function() {
	...
	$('#edit_section').ajaxForm({
      ...
      error: defaultErrorCallback,     
      url: $.api.url +'section/'+$('#edit_section').attr('id_section'),
      dataType: 'json',    
      data: $(this).serializeArray(), 
      type:  'PUT',   
	...
	//$('#edit_section').submit(function() { 	
		
		//var postData = $(this).serializeArray();		
		//var slug = $('#slug', this).val();	
		//var id = $('#edit_section').attr('id_section');				 
				
	    //$.api.put('section/'+id,postData).success(function(response) {		
...
$(function() {
  var l = ['emoticons', 'settings', 'popups', 'ajaxBoard', 'ajaxMessages', 'ajaxUsers','messages', 'users','share'];
